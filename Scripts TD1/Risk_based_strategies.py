...
def inv_volatilités_entreprises(rendements, init, mode='ERB', nombre_mois_retour=1): 
    ...
    for j in range (1,m0):
        ...
        for i in range (len(pos_début_mois)-nombre_mois_retour-1):
            ...
            if (vol == 0):
                #print("vol = 0 pour l'entreprise", j, "au mois", i)
                poids_entreprise.append(0)
...
def pondérations_MV(rendements, init, nombre_mois_retour=1):
    ...
    for i in range (len(pos_début_mois)-nombre_mois_retour-1):
        ...
        print("Taille de la matrice de covariance nettoyée du mois", i ,taille_mcin)
        print("Rang de la matrice nettoyée :", np.linalg.matrix_rank(matrice_covariance_i_nettoyée))
        matrice_covariance_i_inverse = np.linalg.inv(matrice_covariance_i_nettoyée)
        print("Matrice de covariance du mois", i, "inversée avec succès.")
...
def ibs_réf(rendements, n, supervision='no', nombre_mois_retour=1, Mk_risk_constraint=0.01):      # Cette fonction va tracer les différents indices boursiers de référence : EW, ERB, IV.
    ...
    chemin_etf = "./etf_data.csv"
    valeurs_etf = pd.read_csv(chemin_etf, sep=",",lineterminator='\n',skip_blank_lines=False, low_memory=False)
    taille = rendements.shape
    n0 = taille[0]
    taille2 = valeurs_etf.shape
    m0 = taille2[0]
    ib_EW = [1]
    ib_ERB = [1]
    ib_IV = [1]
    ...
    # Petite partie pour normaliser nos ETF
    etf_SPY = valeurs_etf.iloc[m0-n-1-20:m0-1-20,1]
    etf_IVV = valeurs_etf.iloc[m0-n-1-20:m0-1-20,2]
    etf_VOO = valeurs_etf.iloc[m0-n-1-20:m0-1-20,3]
    etf_SPY = etf_SPY.tolist()
    etf_IVV = etf_IVV.tolist()
    etf_VOO = etf_VOO.tolist()
    etf_SPY_norm = [x/etf_SPY[0] for x in etf_SPY]
    etf_IVV_norm = [x/etf_IVV[0] for x in etf_IVV]
    etf_VOO_norm = [x/etf_VOO[0] for x in etf_VOO]
    
    # Nous allons maintenant calculer les poids de nos différents indices.
    init = n0-n-1
    ...
    df = pd.DataFrame({'ib_EW': ib_EW, 'ib_ERB': ib_ERB, 'ib_IV': ib_IV, 'ib_MV': ib_MV, 'ib_Mk': ib_Mk, 'etf_SPY': etf_SPY_norm, 'etf_IVV': etf_IVV_norm, 'etf_VOO': etf_VOO_norm}, index=liste_dates)
    name_file = "Indices_"+str(n)+"j_risque=" + str(Mk_risk_constraint) + ".csv"
    df.to_csv(name_file, index=True)
    plt.figure(figsize=(12, 7))
    ...
    plt.plot(df.index, df['etf_SPY'], marker='o', markersize=1, linestyle='-', label="etf_SPY")
    plt.plot(df.index, df['etf_IVV'], marker='o', markersize=1, linestyle='-', label="etf_IVV")
    plt.plot(df.index, df['etf_VOO'], marker='o', markersize=1, linestyle='-', label="etf_VOO")
    plt.gca().xaxis.set_major_formatter(plt.matplotlib.dates.DateFormatter('%Y-%m-%d'))  # Format AAAA-MM-JJ
    intervalle = int(n/10)
    plt.gca().xaxis.set_major_locator(plt.matplotlib.dates.DayLocator(interval=intervalle))  # Afficher une date tous les 10 jours
    plt.xticks(rotation=45)
...
def comparaisons_ibs(ibs, confidence_level=0.95):
    dico_indicateurs = {"Nom de l'indice": ("perf_moy_ann", "vol_ann", "vol_ann_mens", "max_drawdown", "second_max_drawdown", "prop_max_drawdown", "prop_second_max_drawdown", "VaR" + str(100*confidence_level) + "%", "ES" + str(100*confidence_level) + "%")}
    names_ib = ["ib_EW", "ib_ERB", "ib_IV", "ib_MV", "ib_Mk", "etf_SPY", "etf_IVV", "etf_VOO"]
    for i in range (1,6):
        ib = ibs.iloc[:,[0,i]]
        ...
        ES = Ir.calcule_ES(ib)
        dico_indicateurs[names_ib[i-1]] = (perf_moy, vol, vol_mens, max_drawdown, second_max_drawdown, prop_max_drawdown, prop_second_max_drawdown, VaR, ES)
...
#path = "C:/Users/Trist/Documents/Cours 2A - CS/Semestre 8/ST7 - Finance stochastique/Projet ST7/data/returns.csv"
#path = "./rendements_réduits.csv"
#rendements = pd.read_csv(path, sep=",",lineterminator='\n',skip_blank_lines=False, low_memory=False)

#ib_EW(rendements, 400)
#temps_début = time.time()
#ibs_réf(rendements, 400, supervision='yes', nombre_mois_retour=6, Mk_risk_constraint=0.01)
#temps_fin = time.time()
#temps_exécution = temps_fin - temps_début
#print("Temps d'exécution :", temps_exécution, "secondes")


path_ib = "C:/Users/Trist/Documents/Cours 2A - CS/Semestre 8/ST7 - Finance stochastique/Projet ST7/Indices_400j_risque=0.01.csv"
